...
// ================================================================
{
  ...
  const framework = [
    ...
    { step: "③", label: "回归分析", desc: "面板FE + 分位数回归" },
    { step: "④", label: "空间计量", desc: "Moran's I 空间聚集" },
  ];
...
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现一：转移支付依赖度的空间格局");
...
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现二：全国趋势与依赖度时序演变");

  if (imgTrend) s.addImage({ data: imgTrend, x: 0.1, y: 0.9, w: 4.8, h: 3.5 });
  if (imgBoxplot) s.addImage({ data: imgBoxplot, x: 5.1, y: 0.9, w: 4.7, h: 3.5 });
...
// ================================================================
// Slide 7: 过渡页 — 分组对比与统计检验
// ================================================================
transitionSlide("第二部分：分组对比与统计检验");
...
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现三：三类财政自给率省份的关键差异");
...
// ================================================================
// Slide 9: 结果④ — 转移支付与经济发展
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现四：转移支付与经济发展水平的关系");

  if (imgScatter) s.addImage({ data: imgScatter, x: 0.1, y: 0.9, w: 10.0, h: 4.2 });
...
// ================================================================
{
  ...
  const stats = [
    ...
    { step: "③", label: "Spearman ρ", value: "ρ = -0.8129, p < 0.001", detail: "单调关系极显著" },
    { step: "④", label: "单因素 ANOVA", value: "F = 62.10, p < 0.001", detail: "三类省份差异极显著" },
    { step: "⑤", label: "事后比较", value: "高/中/低 两两比较 p < 0.001", detail: "Bonferroni校正全部显著" },
  ];
  stats.forEach((st, i) => {
    const sy = 1.1 + i * 0.72;
...
// Slide 11: 过渡页 — 回归分析
// ================================================================
transitionSlide("第三部分：回归分析 — 因果识别");

// ================================================================
// Slide 12: 结果⑥ — 面板固定效应回归
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现六：面板固定效应回归");

  if (imgPanelReg) s.addImage({ data: imgPanelReg, x: 0.1, y: 0.9, w: 10.0, h: 4.2 });
...
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现七：分位数回归 — 异质性分析");

  if (imgQReg) s.addImage({ data: imgQReg, x: 0.1, y: 0.9, w: 10.0, h: 4.2 });
...
// ================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  sectionTitle(s, "发现八：空间聚集特征 — Moran's I检验");

  if (imgTimeline) s.addImage({ data: imgTimeline, x: 0.1, y: 0.9, w: 10.0, h: 4.2 });
...
// ================================================================
{
  ...
  const findings = [
    ...
    "③ 整体依赖加深：全国平均依赖度从37%升至47%",
    "④ 三类划分有效：ANOVA F=62.10，组间差异极显著",
  ...
  const policies = [
    ...
    "③ 建立动态监测体系：面板数据年度跟踪，差异化帮扶",
    "④ 考虑空间溢出效应：高依赖集群区域需协同政策支持",
    "⑤ 完善分配公式：纳入人均GDP、人口、公共服务成本差异",
...
// ================================================================
{
  ...
  const insights = [
    { num: "1", title: "AI降低技术门槛，人定义问题边界", desc: "选题方向、指标体系、结论判断均由人拍板" },
    { num: "2", title: "领域知识是不可替代的质量底线", desc: "计划单列市跳过规则、决算滞后回退逻辑——人的判断是校准器" },
    { num: "3", title: "迭代优于一次到位，兜底方案至关重要", desc: "可视化模块5轮调试：Plotly→geopandas PNG" },
    { num: "4", title: "计算思维的核心是问题分解与验证", desc: "分解→协作→验证的循环才是课程核心收获" },
  ];
